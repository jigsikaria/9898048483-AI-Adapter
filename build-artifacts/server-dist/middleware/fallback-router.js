...
export class FallbackRouter {
    ...
    static defaultPredicate = (status, bodyText) => {
        if (status === 400 && bodyText.toLowerCase().includes('context_length_exceeded'))
            return true;
        return status >= 400;
    };
    buildHeaders(provider, key, headers) {
    ...
    rewriteUrl(provider, baseUrl) {
        if (provider === 'ollama') {
            return `${baseUrl}/api/chat`;
        }
        return `${baseUrl}/chat/completions`;
    }
    async buildUpstreamRequest(provider, request, headers, key) {
        const providerConfig = providers[provider];
        ...
        if (provider === 'ollama') {
            const messages = [
                ...request.messages.filter((m) => m.role === 'system').map((m) => ({ role: 'system', content: m.content })),
