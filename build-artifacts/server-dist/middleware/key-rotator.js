import { keyFromEnv } from "../config.js";
/**
...
export class MultiKeyRotator {
    ...
    next(provider) {
        ...
        const oldest = pool.reduce((acc, k) => {
            if (!acc || k.cooldownUntil < acc.cooldownUntil)
                return k;
            return acc;
        }, null);
        if (!oldest)
    ...
    reload(provider) {
        const targets = provider ? [provider] : [...this.keys.keys()];
...
//# sourceMappingURL=key-rotator.js.map