...
export function detectProvider(model: string, header?: string): ProviderName {
  if (header) {
    const normalized = header.toLowerCase() as ProviderName;
  ...
  if (lower.includes('llama') || lower.includes('mixtral') || lower.includes('qwen')) return 'groq';
